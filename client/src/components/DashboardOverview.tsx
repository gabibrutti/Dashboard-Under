  ...
  return (
    <div className="min-h-screen">
      ...
      {!credentials && !isLoading && (
        <div className="flex min-h-screen items-center justify-center bg-gray-50">
          <div className="text-center">
            <p className="text-lg font-semibold text-slate-600">Configure as credenciais para começar</p>
            <p className="mt-2 text-sm text-slate-500">O modal de configuração deve aparecer automaticamente</p>
          </div>
        </div>
      )}
        ...
        {showTelefonia && (
          <section className="mt-12 space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {computed.metrics
                .filter(metric => metric.source === "zenvia")
                .map(metric => (
                  <KPICard key={metric.id} metric={metric} />
                ))}
            </div>
