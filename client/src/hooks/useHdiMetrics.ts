...
import type { HdiMetrics } from "@/types/dashboard";

export function useHdiMetrics({
  startDate,
  endDate,
  groupId,
  agentId,
  ...
  const [data, setData] = useState<HdiMetrics | null>(null);
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/api/hdi-metrics?${params.toString()}`);

      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: "Erro desconhecido ao carregar métricas HDI" }));
        throw new Error(err.error || "Erro ao carregar métricas HDI");
      }
      
      const contentType = res.headers.get("content-type") || "";
      if (!contentType.includes("application/json")) {
        throw new Error("Resposta inválida do servidor ao carregar métricas HDI");
      }
      
      const json = await res.json();
      setData(json as HdiMetrics);
    } catch (err: any) {
      console.error("[Hook] Erro ao carregar métricas HDI:", err);
      setError(err.message || "Erro ao carregar métricas HDI");
      setData(null);
    } finally {
      setIsLoading(false);
    }
